...
describe("profile editing", () => {
  ...
  it("rejects empty names and future birth dates before writing", async () => {
    const upsert = vi.fn();
    const service = new UsersService(
      { userProfile: { upsert } } as unknown as PrismaService,
      {} as EvidenceService,
    );
    await expect(
      service.updateProfile("user-1", { ...body, fullName: " " }),
    ).rejects.toThrow();
    await expect(
      service.updateProfile("user-1", { ...body, dateOfBirth: "2999-01-01" }),
    ).rejects.toThrow();
  ...
  it("reads only the authenticated owner's PROFILE picture", async () => {
    ...
    const prisma = {
      userProfile: {
        findUnique: vi
          .fn()
          .mockResolvedValue({ avatarObjectKey: "profile/user-1/pic.enc" }),
      },
      evidenceFile: { findFirst },
    };
    const service = new UsersService(
      prisma as unknown as PrismaService,
      { read } as unknown as EvidenceService,
    );
    expect(await service.avatar("user-1")).toEqual({
      imageBase64: Buffer.from("picture").toString("base64"),
    });
    expect(findFirst).toHaveBeenCalledWith({
      where: {
        objectKey: "profile/user-1/pic.enc",
        ownerId: "user-1",
        ownerType: "USER",
        purpose: "PROFILE",
        status: "CLEAN",
