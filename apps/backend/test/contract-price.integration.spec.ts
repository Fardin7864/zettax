...
suite("durable contract observation archive", () => {
  ...
  it("recovers the identical observation after restart without calling the provider", async () => {
    ...
    const fetch = vi.fn().mockResolvedValue({
      ...
      json: () =>
        Promise.resolve([
          [start, "1", "1", "1", "100.12345678", "1", start + 999],
        ]),
    });
    vi.stubGlobal("fetch", fetch);
    const first = await new ContractPriceService(prisma).at(
      instrument,
      boundary,
    );
    fetch.mockRejectedValue(new Error("offline"));
    const recovered = await new ContractPriceService(prisma).at(
      instrument,
      boundary,
    );
    ...
    await expect(
      prisma.contractPriceObservation.update({
        where: {
          source_timestamp: {
    ...
    await expect(
      prisma.contractPriceObservation.delete({
        where: {
          source_timestamp: {
            source: first.providerId,
            timestamp: first.timestamp,
