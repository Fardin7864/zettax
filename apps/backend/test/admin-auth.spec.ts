...
describe("separate administrator identity", () => {
  ...
  it("accepts admin sessions and rejects customer sessions", async () => {
    expect((await session.verify(await session.sign("admin-test"))).sub).toBe(
      "admin-test",
    );
    const customer = await jwt.signAsync(
      { sub: "customer" },
      { secret, audience: "primevest-mobile", issuer: "primevest-api" },
    );
