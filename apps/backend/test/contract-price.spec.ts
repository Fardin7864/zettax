...
describe("expiry observation identity", () => {
  ...
  it("requests only the closed second before expiry and keeps its exact decimal", async () => {
    const fetch = vi
      ...
      .mockResolvedValue({
        ok: true,
        json: () => Promise.resolve([[9000, "1", "1", "1", "77123.12345678", "1", 9999]]),
      });
  ...
  it("rejects a later price rather than substituting it", async () => {
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValue({
          ok: true,
          json: () => Promise.resolve([[10000, "1", "1", "1", "100", "1", 10999]]),
        }),
    );
    await expect(
  ...
  it('shares simultaneous requests for the same observation', async () => {
    const fetch=vi.fn().mockResolvedValue({ok:true,json:()=>Promise.resolve([[9000,'1','1','1','100','1',9999]])});
    vi.stubGlobal('fetch',fetch);
    const service=new ContractPriceService();
    await Promise.all(Array.from({length:50},()=>service.at(instrument,new Date(10500))));
    expect(fetch).toHaveBeenCalledTimes(1);
  });
  it('retries the identical timestamp after publication lag', async () => {
    const fetch=vi.fn().mockResolvedValueOnce({ok:true,json:()=>Promise.resolve([])}).mockResolvedValueOnce({ok:true,json:()=>Promise.resolve([[9000,'1','1','1','100','1',9999]])});
    vi.stubGlobal('fetch',fetch);
  ...
  it('rejects instruments without an approved source', async () => {
    await expect(new ContractPriceService().at({baseAsset:'EUR',assetClass:'FOREX'},new Date())).rejects.toMatchObject({code:'CONTRACT_PRICE_UNAVAILABLE'});
  });
});
