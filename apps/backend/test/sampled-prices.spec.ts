...
function setup(config: Record<string, string> = {}) {
  ...
  const prisma = {
    sampledPrice: {
      deleteMany: vi.fn(async ({ where }) => {
        const old = rows.filter((row) => row.sampledAt <= where.sampledAt.lte);
        for (const row of old) rows.splice(rows.indexOf(row), 1);
      ...
      findFirst: vi.fn(
        async ({ where }) =>
          rows.find(
            (row) =>
              row.instrumentSlug === where.instrumentSlug &&
              row.sampledAt >= where.sampledAt.gte,
          ) ?? null,
      ),
      ...
      findMany: vi.fn(async ({ where, take }) =>
        rows
          .filter(
            (row) =>
              row.instrumentSlug === where.instrumentSlug &&
              row.sampledAt > where.sampledAt.gt &&
              (!where.sampledAt.lt || row.sampledAt < where.sampledAt.lt),
          )
  ...
  return {
    service: new SampledPriceService(
...
describe("price-only API snapshots", () => {
  ...
  it("samples seven non-crypto symbols once per 15-minute bucket and deletes old rows", async () => {
    const fetcher = vi
      ...
      .mockImplementation(
        async () =>
          new Response(JSON.stringify({ price: "238.47" }), { status: 200 }),
      );
    vi.stubGlobal("fetch", fetcher);
    ...
    rows.push({
      ...
      sampledAt: new Date(now.getTime() - 31 * 24 * 60 * 60_000),
      provider: "old",
    });
    await service.poll(now);
    await service.poll(new Date(now.getTime() + 60_000));
    ...
    const series = await service.candles(
      instruments.find((item) => item.id === "aapl")!,
      "1m",
      90,
    );
  ...
  it("aggregates only saved samples into the selected interval and excludes old history", async () => {
    ...
    rows.push({
      ...
      sampledAt: new Date(hour + 901_000),
      provider: "TWELVE_DATA_PRICE",
    });
    rows.push({
      id: 3n,
      ...
      sampledAt: new Date(hour + 1_801_000),
      provider: "TWELVE_DATA_PRICE",
    });
    rows.push({
      id: 4n,
      ...
      sampledAt: new Date(now - 31 * 24 * 60 * 60_000),
      provider: "old",
    });
    const series = await service.candles(
      instruments.find((item) => item.id === "aapl")!,
