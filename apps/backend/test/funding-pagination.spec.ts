...
} from "../src/funding/funding-pagination.dto";
...
describe("paginated admin funding history", () => {
  for (const kind of ["deposit", "withdrawal"] as const) {
    it(`includes every ${kind} status by default and uses a bounded stable page`, async () => {
      ...
      const result =
        kind === "deposit"
          ? await service.listDepositsForReview(undefined, 2, 25)
          : await service.listWithdrawalsForReview(undefined, 2, 25);
      ...
      expect(result).toMatchObject({
        ...
        pageSize: 25,
        totalPages: 3,
        items: [{ id: "request", virtualFunding: true }],
      });
    });
    it(`filters ${kind} history and counts with the same status`, async () => {
      ...
      const prisma = {
        $transaction: vi.fn(async (work) =>
          work({ [`${kind}Request`]: { findMany, count } }),
        ),
      };
      const service = new FundingService(
        prisma as never,
      ...
      const status =
        kind === "deposit" ? DepositStatus.CREDITED : WithdrawalStatus.PAID;
      const result =
        kind === "deposit"
          ? await service.listDepositsForReview(DepositStatus.CREDITED)
          : await service.listWithdrawalsForReview(WithdrawalStatus.PAID);
      expect(findMany).toHaveBeenCalledWith(
  ...
  it("validates page bounds, page size and status before querying", async () => {
    expect(
      await validate(
        plainToInstance(AdminDepositQueryDto, {
          page: "2",
          pageSize: "50",
          status: "CREDITED",
        }),
    ...
    for (const query of [
      ...
      expect(
        (await validate(plainToInstance(AdminWithdrawalQueryDto, query)))
          .length,
      ).toBeGreaterThan(0);
    }
  });
