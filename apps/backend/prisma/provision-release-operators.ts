...
async function main() {
  ...
  const records =
    input &&
    typeof input === "object" &&
    Array.isArray((input as { operators?: unknown }).operators)
  ...
  const assignedReleaseRoles = new Set(
    records.flatMap((record) =>
      record.roles.filter((role) => role.startsWith("release-")),
    ),
  );
  if (
    ...
    throw new Error(
      "Every release authority must be assigned exactly through the operator plan.",
    );
  }
  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      if (existing.length) {
        throw new Error(
          `Operator already exists: ${existing.map((row) => row.email).join(", ")}`,
        );
      }
      ...
      for (const [name, permissionKeys] of Object.entries(permissionSets)) {
        const role = await tx.role.upsert({
          ...
          create: {
            name,
            description: `Independent ${name.replace("-", " ")}`,
          },
        });
