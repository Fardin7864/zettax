...
export class SandboxMarketDataProvider extends HttpMarketDataProvider {
  ...
  constructor(private readonly prices: DemoPriceService) {
    super();
  }
  supports(instrument: SeedInstrument) {
  ...
  getCandles(
    ...
    const mark = (time: number) =>
      this.prices.mark(
        { slug: instrument.id, pricePrecision: instrument.pricePrecision },
        new Date(time),
      ).price;
    const format = (value: Prisma.Decimal) =>
      value.toFixed(instrument.pricePrecision);
    const candles: Candle[] = [];
    for (let index = limit - 1; index >= 0; index--) {
      const openMs = lastOpen - index * duration;
      const closeMs = openMs + duration;
      const open = mark(openMs),
        mid = mark(openMs + Math.floor(duration / 2)),
        close = mark(Math.min(closeMs - 1, cutoff - 1));
      const high = Prisma.Decimal.max(open, mid, close),
        low = Prisma.Decimal.min(open, mid, close);
      candles.push({
